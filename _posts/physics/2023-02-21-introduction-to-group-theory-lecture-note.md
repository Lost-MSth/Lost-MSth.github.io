...
tags: ["physics"]
modify_date: "2023-02-25 00:30:00"
aside:
    toc: true
---
...
1. 正规子群的左陪集和右陪集相同，逆命题也成立，注意这并不意味着元素可交换
2. 正规子群由群 $G$ 的若干个完整的类构成，逆命题也成立
3. 正规子群的一个陪集和另一个陪集的成绩（包括自乘）必为一个陪集或正规子群
...
设 $G^{x}$ 是 $G$ 对 $x \in X$ 的迷向子群，则 $G^{x}$ 的每一个左陪集，把点 $x$ 映射为 含 $x$ 的 $G$ 轨道中的一个特定的点 $y$
